---
title: Share
description: Share your Lunos conversations.
---

Lunos's share feature allows you to create public links to your Lunos conversations, so you can collaborate with teammates or get help from others.
...
When you share a conversation, Lunos:
...
Lunos supports three sharing modes that control how conversations are shared:

---

### Manual (default)

By default, Lunos uses manual sharing mode. Sessions are not shared automatically, but you can manually share them using the `/share` command:
...
Lunos is self-hostable by design, so an organisation controls its own deployment and
the sharing behaviour that goes with it.
