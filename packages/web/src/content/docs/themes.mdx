...
With Lunos you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Lunos uses our own `opencode` theme.
...
Lunos comes with several built-in themes.
...
| `high-contrast`        | Pure black or white background; text and status colours at 7:1 contrast or more (WCAG AAA) |
...
- Want Lunos to match their terminal's appearance
...
Lunos supports a flexible JSON-based theme system that allows users to create and customize themes easily.
