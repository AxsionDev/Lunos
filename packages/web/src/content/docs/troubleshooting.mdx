...
To debug issues with Lunos, start by checking the logs and local data it stores on disk.
...
To uninstall the Lunos CLI and remove its related files, run:
...
To remove Lunos Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

Lunos Desktop runs a local Lunos server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `Lunos` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
Lunos can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so Lunos can rebuild it.

1. Quit Lunos Desktop completely.
...
3. Restart Lunos Desktop.

---

### Fix server connection issues

Lunos Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, Lunos Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with Lunos's features.

---

### Notifications not showing

Lunos Desktop only shows system notifications when:

- notifications are enabled for Lunos in your OS settings, and
...
1. Quit Lunos Desktop.
2. Find and delete these files (they live in the Lunos Desktop app data directory):
...
If you're experiencing issues with Lunos:
