---
title: CLI
description: Lunos CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Lunos CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with Lunos programmatically.
...
Start the Lunos terminal user interface.
...
The Lunos CLI also has the following commands.

---

### agent

Manage agents for Lunos.
...
Attach a terminal to an already running Lunos backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote Lunos backend. For example:
...
Lunos is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When Lunos starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Lunos.
...
Start a headless Lunos server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
Show token usage and cost statistics for your Lunos sessions.
...
Review long-term memory (see [Rules: long-term memory](/docs/rules#long-term-memory)). `list`, `show` and `export` read what is stored without starting memory.

```bash
opencode memory list [--scope project|user]
opencode memory show <id>
opencode memory search <query>
opencode memory forget <id>
opencode memory purge --scope project|user --yes
opencode memory export [--scope project|user] [--dir <path>]
```

- `export` writes one Markdown file per fact, with its provenance, to `.opencode/memory/export/` so you can review memory in git.
- `forget` removes the fact from memory and from that export folder.
- `purge` deletes a memory's stored facts and graph. Your notes in `.opencode/memory/*.md` are kept.
...
Import session data from a JSON file or Lunos share URL.

```bash
opencode import <file>
```

You can import from a local file or an Lunos share URL.
...
Start a headless Lunos server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access Lunos through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run Lunos.
...
Uninstall Lunos and remove all related files.
...
Lunos can be configured using environment variables.
