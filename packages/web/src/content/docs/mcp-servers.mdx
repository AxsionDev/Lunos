...
You can add external tools to Lunos using the _Model Context Protocol_, or MCP. Lunos supports both local and remote servers.
...
You can define MCP servers in your [Lunos Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Marketplaces can list MCP servers alongside plugins, so you can find a server without already knowing its connection details. The Lunos community marketplace ships a starter set, including the official reference servers.

```bash
lunos marketplace add AxsionDev/Lunos
lunos mcp search docs
lunos mcp add context7
```

`lunos mcp add <name>` shows the command it will run (or the URL it will connect to) and the config file it will write, then asks for confirmation. Pass `--yes` to skip the prompt. If two marketplaces list the same name, qualify it as `<marketplace>/<name>`.

A marketplace entry only declares the _names_ of the environment variables or headers a server needs. Lunos writes them as `{env:NAME}` references, so the generated config holds no secrets. Export the variable before starting Lunos.
...
Lunos automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Lunos will:
...
If the server requires authentication, Lunos will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Lunos will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Lunos, alongside built-in tools. So you can manage them through the Lunos config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Lunos to your Sentry account.
