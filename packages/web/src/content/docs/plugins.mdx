---
title: Plugins
description: Write your own plugins to extend Lunos.
---

Plugins allow you to extend Lunos by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify Lunos's default behavior.
...
**Marketplace manifests** (added via `lunos marketplace add`) are likewise cached to disk, in `~/.cache/opencode/marketplace/`, so `marketplace list`, `plugin list`/`search`, and the TUI Discover view don't re-fetch on every run. Run `lunos marketplace update <name>` to force a refresh.

**Marketplaces** can list plugins, skill sources, hooks and MCP servers. Search all of them with `lunos marketplace search <query> [--kind plugin|skill|hook|mcp]`, and install one by name with `lunos marketplace install <name>` (or `lunos plugin add <name>` for plugins). In the TUI, the Discover view has a tab per kind; press `ctrl+o` to switch. Hooks and MCP servers show exactly what they will run, and ask before anything is written to your global config.
...
Lunos runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the Lunos desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
