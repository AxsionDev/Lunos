...
Only these event names are recognised. **A misspelled event is silently ignored** — config
decoding drops unknown keys rather than rejecting them, so the hook simply never fires and
you get no warning. If a hook seems to do nothing, check the event name against this table
first.
