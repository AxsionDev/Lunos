---
title: TUI
description: Using the Lunos terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Lunos provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Lunos starts the TUI for the current directory.
...
When using the Lunos TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Lunos. Allows you to select from available providers and add their API keys.
...
- `leader_timeout` - Controls how long Lunos waits after the leader key. Defaults to `2000`.
- `scroll_acceleration.enabled` - Enable macOS-style scroll acceleration for smooth, natural scrolling. When enabled, scroll speed increases with rapid scrolling gestures and stays precise for slower movements. **This setting takes precedence over `scroll_speed` and overrides it when enabled.**
- `scroll_speed` - Controls how fast the TUI scrolls when using scroll commands (minimum: `0.001`, supports decimal values). Defaults to `3`. **Note: This is ignored if `scroll_acceleration.enabled` is set to `true`.**
- `diff_style` - Controls diff rendering. `"auto"` adapts to terminal width, `"stacked"` always shows a single-column layout.
- `reduced_motion` - `true` turns off every animation (the home screen's moon reflection, spinners, placeholder cycling), and the in-app "Disable animations" toggle can't turn them back on. When unset, animations are off by default if `NO_COLOR` is set or `TERM=dumb`, and the toggle decides.

For a high-contrast display, set `"theme": "high-contrast"` (see [Themes](/docs/themes)).
