...
- [ ] **Step 4:** Create IIS site `Lunos.Web` (physical path `C:\inetpub\lunos-web`, binding `https://lunos.tech` + `http://lunos.tech` redirecting to https, app pool `Lunos.Web`). Add a `web.config` under `C:\inetpub\lunos-web` with an IIS URL Rewrite rule that falls back unmatched paths to `index.csr.html`, **not** `index.html` — since Task 11 added build-time prerendering, `index.html` is now the prerendered home page (not a generic SPA shell), and `index.csr.html` is the true empty client-side-rendering shell that unmatched paths need (SPA client-side routing only — not a proxy to the API).
