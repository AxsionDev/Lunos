...
Produce a report with: a summary count by severity, findings grouped by severity (each with **location** `file:line`, **description**, **impact**, **fix**), good patterns observed, and a **Verdict** (`APPROVED` | `NEEDS FIXES`). The dimension reference gives the exact section layout. When writing to a workspace, wrap it per the `agent-output-contract` skill.
