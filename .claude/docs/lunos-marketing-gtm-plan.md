...
1. **Per-release notes** — generated by the pipeline (`script/version.ts` → `script/changelog.ts` →
   `gh release create --notes-file`), attached to each GitHub Release. *What changed in this release.*
2. **`CHANGELOG.md`** at the repo root — curated, hand-written, public. *What changed across
   releases.* Upstream has no root changelog, so this fork-only file carries near-zero merge risk.
3. **Lunos Notes** in `.claude/docs/notes/` — narrative. ***Why we did it that way***, which is the
