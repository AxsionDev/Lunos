...
The implementation keeps **three changelog-shaped artifacts** distinct, rather than collapsing them:

1. **Per-release notes** — generated by the pipeline (`script/version.ts` → `script/changelog.ts` →
   `gh release create --notes-file`), attached to each GitHub Release. _What changed in this release._
2. **`CHANGELOG.md`** at the repo root — curated, hand-written, public. _What changed across
   releases._ Upstream has no root changelog, so this fork-only file carries near-zero merge risk.
3. **Lunos Notes** in `.claude/docs/notes/` — narrative. **_Why we did it that way_**, which is the
   half no generator produces and the half that does the build-in-public work.
