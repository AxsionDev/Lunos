...
| --------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deploy.yml`                | Deploys console/web via SST to AWS + Cloudflare.               | Needs Lunos's own AWS account, SST state, and Cloudflare token. References `anomalyco`. **XCOD-17 settled who owns the accounts (Axsion); XCOD-55 settled the provider question — no hosted offering near-term, so US-controlled providers are acceptable for this workflow _while it deploys only our own console/web_, and the sovereignty claim stays scoped to self-hosted.** Note `deploy.yml:34` targets AWS **`us-east-1`**, a US region — filed separately; fix before un-deferring. Plus XCOD-18 re-pointing. |
