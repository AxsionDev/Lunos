...
For any UI/visual/browser behavior, use the **Gemini → Chrome DevTools → Playwright** tool hierarchy and the **Reproduce → Fix → Verify** workflow (capture a pre-fix baseline screenshot/DOM/console, then verify post-fix against it). Code reading alone cannot reveal runtime state, CSS conflicts, or timing issues.
