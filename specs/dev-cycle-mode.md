...
carried in mode-owned _data_, not in new control flow.
...
1. **No `prompt` field on the agent entry.** For a primary agent, `prompt`
   _replaces_ `SystemPrompt.provider()` (see `session/llm/request.ts` and the
...
The accepted cost: a model _can_ skip a gate, and write access exists before
...
`pending | approved` and describes the gate at the _end_ of the current
phase.

Note the asymmetry between _phases_ and _cursor states_: the cycle has four
...
They are deliberately _not_ project markdown under `.opencode/agent/`, which is
...
"allow"` from `defaults`, so a "degraded" `architect` would be a _writable_
architect — precisely inverting the guarantee below.

Setting `prompt` on these entries does not violate §1's constraint 1. That
constraint is specific to _primary_ agents, where `prompt` replaces
...
- **User config now outranks the agent's own rules.** Native rulesets are built
  ...
  but it narrows the claim above: read-only is a guarantee against the _agent_,
...
| Malformed frontmatter                   | Degrade to `discover`/`pending`; never throw; **the reminder states explicitly that the position is unknown**, so a parse failure does not read as genuinely being at phase 1. Only an unreadable _phase_ triggers that — an unreadable `gate` degrades to `pending` silently, which errs toward re-asking rather than toward redoing work |
